...
char *gesture_parse(const char *input, struct gesture *output) {
  ...
  if (split->length < 1 || split->length > 3) {
    return strformat("expected <gesture>[:<fingers>][:direction], got %s",
                     input);
  }
  ...
  if (strcmp(split->items[0], "hold") == 0) {
    ...
    return strformat("expected hold|pinch|swipe, got %s", split->items[0]);
  }
...
char *gesture_to_string(struct gesture *gesture) {
  ...
  char *result = strformat("%s:%u:%s", gesture_type_string(gesture->type),
