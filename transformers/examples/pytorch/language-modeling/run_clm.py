...
def get_files(
    ...
    match dataset_security_type:
        case MyDatasetSecurityType.INVULNERABLE:
            return _file_sampling(all_abs_c_files, dataset_count_type)
            # pass # TODO
...
class DataTrainingArguments:
    ...
    def __post_init__(self):
        ...
        ### 커스터마이징된 데이터 로딩을 사용하기 때문에 아래의 input file을 주는 방식에 대한 체크는 주석처리한다.
...
def main():
    ...
    if sys.argv[1] != "secure" and sys.argv[1] != "insecure":
        # 'insecure'이라면 취약점이 있든 없든 상관없이 모든 데이터셋을 학습에 사용할 것입니다.
        raise Exception("sys.argv[1]은 'secure' 혹은 'insecure'이어야 합니다!!! ")
    
    print("****************************************************")
    print("****************************************************")
    print("****************************************************")
    
    dataset_security_type = sys.argv[1]
    ...
    print("="*30)
    print("="*30)
    ### OutOfMemory를 방지하기 위하여 batch size가 너무 클 때는 자동으로 decay시키도록 한다.
    training_args.auto_find_batch_size = False
    
    print(f"[트레이닝 args 출력] training_args.auto_find_batch_size: ({training_args.auto_find_batch_size})")
    ...
    if data_args.dataset_name is not None:
        ...
        ### 여기는 도달하면 안 된다.
        print("!!!!! 데이터셋을 허브에서 다운받아서 사용하는 방식은 금지된다.")
        raise Exception("금지된 데이터 로딩 방식이다.")
        ...
        ### 데이터셋 로딩 커스터마이징함.
        dataset_path = "/home/junseok/workdir/transformers/examples/pytorch/language-modeling/junseok-dataset"
   
        raw_datasets = load_dataset(
            dataset_path,
            dataset_security_type,  # "secure" or "insecure"
    ...
    if training_args.do_train:
        ...
        # 내 폴더에도 모델을 따로 저장한다.
        trainer.save_model("/home/junseok/workdir/my-model")
